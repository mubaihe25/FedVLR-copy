...
- `configs/workbench_experiment_schema.json` defines the bounded frontend workbench options for four directions: recommendation manipulation, membership inference, update leakage, and aggregation defense. It now exposes `execution_modes`, `parameter_descriptors`, and `model_dataset_execution` so the frontend can render execution mode, dynamic parameters, and model/dataset capability hints from the backend schema.
...
- `execution_mode=existing_artifact` reuses existing V3 artifacts and must return `source=existing_artifact`; `execution_mode=real_smoke` requests a bounded runner path and must fail validation when the selected direction/model/dataset has no white-listed real-smoke path; `execution_mode=probe_smoke` is reserved for explicitly requested lightweight risk probes and must not be described as full training.
- For `recommendation_manipulation` on `AMAZON_BEAUTY_POC` with `FedAvg`, the runner may launch the white-listed Amazon target-injection smoke and return `source=real_smoke`. For `aggregation_defense` on `KU` with `FedAvg` or `FedRAP`, it may launch a real 1-epoch white-listed smoke via `scripts/launch_experiment.py` and return `source=real_smoke`. Both are small link validation jobs, not full attack/defense benchmarks.
- Membership inference and update leakage do not silently downgrade from `real_smoke` to probe. Users must explicitly request `execution_mode=probe_smoke` for the bounded probe/result回填 path, and it must not be described as freshly trained output. Aggregation defense can return `partial` when only config-only defense evidence exists.
- Launchable model choices are limited to FedAvg, FedRAP, FedNCF, FCF, MMFedAvg, MMFedRAP, MMFedNCF, and MMFCF. All eight may enter workbench configuration for both Amazon Beauty and KU, but `model_dataset_execution` and validate responses decide whether the selected direction can run `real_smoke`, only reuse evidence, or only pass configuration/probe validation. FCF/MMFCF remain partial. MGCN, MMGCN, MMMGCN, and MultiModalMGCN are adapter-required and are not launchable from the workbench.
...
- `/workbench/options` consumers rely on `execution_modes`, `direction_parameters`, `defense_parameters`, `compatibility_matrix`, `model_dataset_execution`, `parameter_descriptors`, and Amazon target item fields such as `raw_title`, `display_name_zh`, `short_name_zh`, `category_zh`, `image_url`, and `thumbnail_url`. `compatibility_matrix` is a selectable-config matrix; do not treat it as proof that every model/dataset can run real smoke.
- Validation returns `field_errors` for UI-level feedback. Malicious-client ratio may be 0; secure aggregation and robust aggregators are mutually exclusive; TrimmedMean/Krum/Bulyan parameters are only valid when their algorithms are selected.
