...
- New jobs write `metrics_summary.json` with `schema_version=workbench-result-v2`, identity/timestamps, `config_summary`, common `training` metrics/rounds, direction-only `direction_result`, `warnings`, `missing_evidence`, and `partial_reason`. Paths in job artifacts stay repository-relative.
- Recommendation manipulation exports the current job target metadata, baseline/attack ranks, Top50 hit, baseline/attack/optional-defense recommendation items, list counts/change statistics, Jaccard, and the documented target-manipulation index. A defense list is absent when no defense phase ran.
- Membership inference runs after the current job training and exports AUC, Accuracy, Precision, Recall, F1, score gap, threshold, evidence/label/model policies, sample counts, ROC points, score distributions, and an anonymized pair-score CSV. Explicitly unavailable evidence produces `partial`; it is not replaced by showcase data.
- Update leakage evaluates only the configured audit-client subset and exports real candidate ranks/scores, anonymous per-client true interactions, Hit@10/20/50, modality/source/similarity settings, and `update_leakage_candidate_evidence.json` when export is enabled.
- Aggregation defense uses separate clean baseline, attacked-without-defense, and same-seed defended launcher phases when `base_attack=malicious_update`. It exports phase metrics, recovery math, accepted/rejected/filter statistics, anonymized rejected-client IDs, and aligned per-round Loss/Recall@50/NDCG@50/filter evidence. `base_attack=none` does not invent attack/filter success fields.
- Code executability and runtime verification are distinct capability fields. FedAvg + Amazon Beauty has completed real one-epoch `full_train` validation for all four workbench directions; this does not generalize to other model/dataset pairs or final model quality.
