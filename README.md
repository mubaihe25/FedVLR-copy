...
- `scripts/cache_amazon_item_images.py` can cache Amazon Beauty PoC product images from local `item_metadata.json` into `datasets/AMAZON_BEAUTY_POC/item_images/` and writes `item_image_manifest.json`. It prioritizes target items, items appearing in recommendation comparison artifacts, and high-interaction items. Download failures are recorded per item and do not fail the run.
