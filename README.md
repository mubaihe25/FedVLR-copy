...
- `execution_mode=existing_artifact` reuses existing V3 artifacts and must return `source=existing_artifact`; `execution_mode=real_smoke` requests a bounded runner path; `execution_mode=probe_smoke` is reserved for lightweight risk probes and must not be described as full training.
...
- Membership inference and update leakage requests for real smoke may be normalized to `source=probe_smoke`; this is a bounded probe/result回填 path and must not be described as freshly trained output. Aggregation defense can return `partial` when only config-only defense evidence exists.
