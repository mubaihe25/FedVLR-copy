...
- `execution_mode=existing_artifact` must keep `source=existing_artifact`; `execution_mode=real_smoke` may only launch white-listed bounded smoke paths; `execution_mode=probe_smoke` is a lightweight risk-probe/result回填 path and must not be called full training.
...
- If a job reuses existing V3 evidence, keep `source=existing_artifact`; if membership inference or update leakage is downgraded to a probe, keep `source=probe_smoke`; do not present either as newly trained output. If aggregation defense is config-only, use `partial` and keep the benchmark boundary visible.
