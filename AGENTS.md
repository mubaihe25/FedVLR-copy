...
- Validation should return field-level errors in `field_errors`. Keep malicious-client ratio 0 valid, keep secure aggregation mutually exclusive with Krum/Median/TrimmedMean/Bulyan, and validate robust-only parameters only when the matching algorithm is selected.
