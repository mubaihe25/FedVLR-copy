...
- `privacy_eval/export_membership_pair_scores.py` can build `membership_pair_scores.csv` from real score files or TopK rank evidence. Rank-only evidence is a proxy and must keep `score_source=rank_proxy`; checkpoint scoring is future adapter work unless explicitly implemented.
- `attacks/target_interaction_injection.py` is planner-only target item promotion. It writes a malicious interaction plan and must not be described as modifying local training data or implementing a full backdoor.
- `privacy_eval/interaction_reconstruction_probe.py` estimates candidate item ids from item-like embedding updates. It is not complete interaction-history recovery, not image reconstruction, and must not save raw participant updates.
