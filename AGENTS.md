...
- `configs/workbench_experiment_schema.json` and `scripts/generate_workbench_smoke_config.py` support the frontend attack-defense workbench. The schema owns `execution_modes`, `parameter_descriptors`, selectable model/dataset choices, and `model_dataset_execution` capability hints.
...
- `execution_mode=existing_artifact` must keep `source=existing_artifact`; `execution_mode=real_smoke` may only launch white-listed bounded smoke paths and must fail validation when unsupported; `execution_mode=probe_smoke` is an explicitly requested lightweight risk-probe/result回填 path and must not be called full training.
- `recommendation_manipulation` on `AMAZON_BEAUTY_POC` with `FedAvg` and `aggregation_defense` on `KU` with `FedAvg` or `FedRAP` may use white-listed `scripts/launch_experiment.py` paths as `source=real_smoke`; keep them capped to 1 epoch / 1 local epoch / 0.05 client sampling and describe them only as small smoke validation.
- If a job reuses existing V3 evidence, keep `source=existing_artifact`; do not downgrade a user-requested `real_smoke` to probe or artifact reuse. If membership inference or update leakage needs the probe path, the request must explicitly use `execution_mode=probe_smoke` and keep `source=probe_smoke`; do not present either as newly trained output. If aggregation defense is config-only, use `partial` and keep the benchmark boundary visible.
- Keep the launchable model whitelist to FedAvg, FedRAP, FedNCF, FCF, MMFedAvg, MMFedRAP, MMFedNCF, and MMFCF unless new model-specific smoke evidence exists. All eight may be selectable for workbench configuration, but validate must be the authority for real-smoke availability. Keep MGCN-family models as adapter-required until trainer/import support is validated.
- Workbench options must stay canonical: datasets are only `AMAZON_BEAUTY_POC` and `KU`; target items should expose stable display fields (`raw_title`, `display_name_zh`, `short_name_zh`, `category_zh`, `image_url`, `thumbnail_url`) for frontend selectors.
- Validation should return field-level errors in `field_errors` and an `error_message`. Keep malicious-client ratio 0 valid, keep secure aggregation mutually exclusive with Krum/Median/TrimmedMean/Bulyan, and validate robust-only parameters only when the matching algorithm is selected. A model being selectable is not proof that the current direction can run `real_smoke`.
