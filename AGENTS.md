...
- V2 matrix generation should also scan bounded real smoke outputs for configs in `configs/experiment_smoke/model_matrix_v2/`. Per model it may report `smoke_status`, `smoke_result_dir`, `topk_export_verified`, `metrics_export_verified`, `security_artifact_ready`, `failure_reason`, and `verification_level`. Do not mark a model as `smoke_verified` unless a 1-epoch smoke produced readable CSV/summary/result JSON and a non-empty `recommend_topk_manifest.json`.
...
## Workbench Full-Training Configs

- `configs/workbench_experiment_schema.json` and `scripts/generate_workbench_smoke_config.py` support the frontend attack-defense workbench. The schema owns `parameter_descriptors`, selectable model/dataset choices, and `model_dataset_execution` capability hints.
- The generator validates and normalizes full-training configs. Its standalone job-writing helper must not start training, delete outputs, or mark a job as completed training.
- `scripts/run_workbench_smoke_job.py` is the whitelisted full-training runner used by FedVLR-API. It may update a job through `queued` / `preparing_config` / `running` / `exporting_artifacts` / `completed` / `partial` / `failed`, but it must not run arbitrary commands or delete outputs.
...
- New workbench requests default to and only accept `execution_mode=full_train`; completed jobs use `source=full_train`. Explicit legacy execution modes must fail validation.
- Unsupported direction/model/dataset combinations must return field-level errors and must never fall back to artifact reads or probe execution.
- The runner must preserve user-submitted epochs, local epochs, client sampling ratio, learning rate, weight decay, gradient clipping, attack parameters, and defense parameters within schema validation bounds. Do not reintroduce runtime caps that silently replace those values.
- Keep the launchable model whitelist to FedAvg, FedRAP, FedNCF, FCF, MMFedAvg, MMFedRAP, MMFedNCF, and MMFCF. Capability checks must distinguish model class, trainer, dataset loader, direction hooks, exporter, and runtime verification; absence of an old job is not an unsupported-code verdict. FedAvg and MMFedRAP recommendation manipulation on Amazon Beauty have real 1-epoch workbench validation. Keep MGCN-family models as adapter-required until trainer/import support is validated.
- Workbench options must stay canonical: datasets are only `AMAZON_BEAUTY_POC` and `KU`; `common_parameters`, `fixed_parameters`, and `parameter_descriptors` are the single source for labels, ranges, steps, defaults, option labels, and dynamic limits. Target items should expose stable display fields (`raw_title`, `display_name_zh`, `short_name_zh`, `category_zh`, `image_url`, `thumbnail_url`) for frontend selectors.
- TopK is not user-configurable in the workbench. The generator and runner must force `top_k=50`, `topk=[50]`, and `recommendation_topk=50`; keep only the Top50 recommendation export switch and the independent audit export switch.
- Validation should return field-level errors in `field_errors` and an `error_message`. Accept at most one Krum/Median/TrimmedMean/Bulyan algorithm, treat an empty selection as ordinary FedAvg aggregation, and keep secure aggregation mutually exclusive with robust aggregation. Krum carries `krum_f`, `multi_krum_enabled`, `distance_metric`, and `gradient_clip_norm`; Median carries `gradient_clip_norm` and `outlier_strategy`; TrimmedMean/Bulyan keep their two dedicated fields. Robust preprocessing `gradient_clip_norm` and DP-style noise `max_grad_norm` are independent. Compute Krum `f <= floor((sampled_clients-3)/2)`, Bulyan `f <= floor((sampled_clients-3)/4)`, and `trim_min_keep <= sampled_clients`; reject invalid values instead of silently replacing them. Aggregation-defense jobs accept only `base_attack=none|malicious_update` and default to `none`.
- DP-style noise remains clipping plus noise without a formal privacy accountant. Its multiplier, clipping bound, recorded delta, and separate perturbation seed must follow the shared descriptors and must not be described as formal differential privacy.
- Recommendation-manipulation full-training jobs must execute separate baseline and attack launcher processes, stream both stdout and stderr into `run.log`, and persist the repository Python path, FedVLR `cwd`, subprocess PID, command, return code, start/finish times, result pointers, Top50 previews, and target-rank comparison. Preserve the first traceback and failure phase on nonzero exit.
- Multimodal models must adapt fusion input projections to the dataset's actual text/vision feature dimensions rather than assuming they equal the ID embedding size.
- New workbench outputs must use `workbench-result-v2`: current-job-only `training` and `direction_result`, relative pointers, explicit `warnings` / `missing_evidence` / `partial_reason`, and no fabricated zero values for absent evidence. Preserve old-job read compatibility without rewriting old outputs.
- Membership inference must export anonymized pair scores and real ROC/distribution metrics from the selected evidence policy. Update leakage must limit captured clients, keep anonymous per-client true-item/candidate evidence, and compute Hit@10/20/50 from those rankings.
- Aggregation defense with `base_attack=malicious_update` must run clean baseline, attacked without robust aggregation, and defended under the same seed/data split. Keep one robust aggregator per job and export aligned per-round metrics plus anonymized filtering evidence. With `base_attack=none`, omit malicious/filter-success claims.
- `status=partial` is reserved for completed training with named missing direction evidence. Launcher warnings alone do not make a job partial. Failed jobs must retain the real phase and error summary/detail.
- FedAvg + Amazon Beauty is runtime-verified for all four one-epoch workbench directions as of 2026-06-14. Keep executability separate from verification and do not extend that claim to other combinations without equivalent jobs.
