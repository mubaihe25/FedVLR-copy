...
DISPLAY_NOTES = {
    "baseline": "正常基线：无攻击、无主动防御，用于提供强攻防实验的推荐性能参照。",
    "attack_only_model_replacement": "强攻击组：恶意客户端执行 minimal model-replacement-like 更新，观察性能退化与攻击计数字段。",
    "attack_and_defense_trimmed_mean": "强攻防组：在 model_replacement 攻击后启用 trimmed_mean，观察鲁棒聚合式防御对极端更新的约束。",
}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the minimal strong adversarial FedVLR experiment matrix."
    )
    parser.add_argument("--model", default="FedRAP")
    parser.add_argument("--dataset", default="KU")
    parser.add_argument("--type", default="StrongAdvMatrix")
    parser.add_argument("--comment-prefix", default="strong_adv_matrix")
    ...
    parser.add_argument(
        "--summary-only",
        action="store_true",
        help="Do not train; rebuild summary files from existing strong-adv outputs.",
    )
    return parser
...
def build_scenarios(args: argparse.Namespace) -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        "attack_only_model_replacement": scenario(
            ...
            goal="验证 minimal model-replacement-like 强攻击对推荐性能和攻击记录字段的影响。",
            config_updates={
                **replacement_attack_config,
                "enabled_defenses": [],
                "enabled_privacy_metrics": [],
        ...
        "attack_and_defense_trimmed_mean": scenario(
            ...
            goal="验证 model_replacement 强攻击下 trimmed_mean 鲁棒聚合式防御是否稳定生效。",
            config_updates={
                **replacement_attack_config,
...
def max_trimmed_mean_round_value(result_payload: Dict[str, Any], key: str) -> Optional[int]:
    values: List[int] = []
...
def max_round_defense_count(result_payload: Dict[str, Any], defense_name: str, key: str) -> int:
    values: List[int] = []
    for round_metric in result_payload.get("round_metrics", []):
        defense_metrics = (
            round_metric.get("extra", {}).get("defense_metrics", {})
...
def build_summary_record(
    ...
    return {
        ...
        "trimmed_mean_effective_trim_count": trimmed_mean_max_trim_count or 0,
    ...
    for record in summary_object["records"]:
        lines.append(
            "| {scenario} | {mode} | {attacks} | {defenses} | {recall} | {ndcg} | {loss} | {malicious} | {attacked} | {clipped} | {filtered} | {trimmed} | {trim_count} | {note} |".format(
                ...
                trim_count=record.get("trimmed_mean_effective_trim_count", 0),
...
def build_strong_adv_summary(args: argparse.Namespace) -> Dict[str, Any]:
    records: List[Dict[str, Any]] = []
    for scenario_name in STRONG_ADV_SCENARIOS:
        paths = find_output_paths(args, scenario_name)
        summary_path = paths["summary"]
        result_path = paths["result"]
        if not summary_path.exists() or not result_path.exists():
            raise FileNotFoundError(
                "Missing strong-adv output for {}: {}, {}".format(
...
def write_summary_outputs(args: argparse.Namespace) -> Dict[str, str]:
    summary = build_strong_adv_summary(args)
    result_dir = ROOT / "outputs" / "results" / args.model / args.dataset / args.type
    result_dir.mkdir(parents=True, exist_ok=True)
    json_path = result_dir / "strong_adv_matrix_summary.json"
    markdown_path = result_dir / "strong_adv_matrix_summary.md"
...
def main() -> int:
    ...
    if args.dry_run:
        dry_run_payload = {
            ...
            for name in STRONG_ADV_SCENARIOS
        }
    ...
    if not args.summary_only:
        for scenario_name in STRONG_ADV_SCENARIOS:
            ...
            print("\n===== Running strong_adv_matrix scenario: {} =====".format(scenario_name))
