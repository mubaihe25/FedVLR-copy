...
COMPATIBILITY_NOTES = {
    "baseline": "MMFedRAP 基线链路验证：确认多模态特征加载、客户端训练、聚合与 JSON/CSV 输出可用。",
    "attack_only_model_replacement": "MMFedRAP 强攻击兼容验证：确认 model_replacement 能处理多模态上传更新结构并记录攻击字段。",
    "attack_and_defense_trimmed_mean": "MMFedRAP 强攻防兼容验证：确认 trimmed_mean 能处理多模态上传更新结构并记录鲁棒聚合字段。",
}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the strong adversarial hook line on MMFedRAP/KU."
    )
    parser.add_argument("--model", default="MMFedRAP")
    parser.add_argument("--dataset", default="KU")
    parser.add_argument("--type", default="MMFedRAPCompat")
    parser.add_argument("--comment-prefix", default="mmfedrap_compat")
    ...
    parser.add_argument(
        "--summary-only",
        action="store_true",
        help="Do not train; rebuild summary files from existing MMFedRAP compat outputs.",
    )
    return parser
...
def build_scenarios(args: argparse.Namespace) -> Dict[str, Dict[str, Any]]:
    ...
    def scenario(
        ...
        return {
            "name": name,
            "goal": COMPATIBILITY_NOTES[name],
            "display_note": COMPATIBILITY_NOTES[name],
            "model": args.model,
...
def build_summary_record(
    ...
    return {
        ...
        "compatibility_note": COMPATIBILITY_NOTES[scenario_name],
    }


def build_mmfedrap_compat_summary(args: argparse.Namespace) -> Dict[str, Any]:
    records: List[Dict[str, Any]] = []
    for scenario_name in MMFEDRAP_COMPAT_SCENARIOS:
        paths = find_output_paths(args, scenario_name)
        summary_path = paths["summary"]
        result_path = paths["result"]
        if not summary_path.exists() or not result_path.exists():
            raise FileNotFoundError(
                "Missing MMFedRAP compat output for {}: {}, {}".format(
    ...
    return {
        ...
        "compatibility_scope": "minimal MMFedRAP strong adversarial hook validation",
        "records": records,
    }
...
def build_markdown(summary_object: Dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "| 场景 | 模式 | 攻击 | 防御 | Recall@20 | NDCG@20 | Loss | malicious | attacked | trimmed_mean | effective_trim_count | 兼容说明 |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
...
def write_summary_outputs(args: argparse.Namespace) -> Dict[str, str]:
    summary = build_mmfedrap_compat_summary(args)
    result_dir = ROOT / "outputs" / "results" / args.model / args.dataset / args.type
    result_dir.mkdir(parents=True, exist_ok=True)
    json_path = result_dir / "mmfedrap_compat_summary.json"
    markdown_path = result_dir / "mmfedrap_compat_summary.md"
...
def main() -> int:
    ...
    if args.dry_run:
        payload = {
            ...
            for name in MMFEDRAP_COMPAT_SCENARIOS
        }
    ...
    if not args.summary_only:
        for scenario_name in MMFEDRAP_COMPAT_SCENARIOS:
            ...
            print("\n===== Running MMFedRAP compat scenario: {} =====".format(scenario_name))
